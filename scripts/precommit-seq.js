...
try {
  const out = execSync('git diff --cached --name-only --diff-filter=ACM').toString();
  ...
  if (js.length) {
    run(`yarn biome check --write ${js.map(f => JSON.stringify(f)).join(' ')}`);
  }
  if (ts.length) {
    run(`yarn biome check --write ${ts.map(f => JSON.stringify(f)).join(' ')}`);
  }
  if (css.length) {
